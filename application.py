...
st.sidebar.image(head, caption="",use_column_width=True)
...
menu=["House Prediction","Predicted House","About"]
...
if choices=='House Prediction':
    ...
    years=tuple([i for i in range(1872,2024)])
    YearBuilt=st.selectbox("Select the built year",years)
    remyears=tuple([i for i in range(1950,2024)])
    YearRemodAdd=st.selectbox("Select Remodel date (same as construction date if no remodeling or additions)",remyears)
    garyears=tuple([i for i in range(1872,2024)])
    garyears=tuple(map(float,garyears))
    GarageYrBlt=st.selectbox("Select year in which Garage was built)",garyears)
    MasVnrArea=st.number_input("Masonry veneer area (in Sqft)",value=0,format='%d',step=1)
    Fireplaces=st.number_input("Select number of FirePlaces",min_value=1,max_value=10,format='%d')
    BsmtFinSF1=st.number_input("Enter the total area(in sqft)",value=0,format='%d')
    ...
    # Perform operations on the uploaded image here
    # You can pass the 'image' variable to your image processing functions
